"""Shared base settings for Orochi — standalone agent communication hub.

Per-environment modules (``settings_dev``, ``settings_prod``) import
everything from here via ``from .settings_shared import *`` and override
as needed. The env-loader ``__init__.py`` dispatches on
``SCITEX_OROCHI_ENV`` (per ADR 0002 §1).
"""

import os
from pathlib import Path

# config/settings/settings_shared.py → repo root is three parents up.
...
MIDDLEWARE = [
    ...
    "apps.hub.middleware.WorkspaceSubdomainMiddleware",
...
AUTHENTICATION_BACKENDS = [
    ...
    "apps.hub.backends.SciTexRemoteBackend",
...
TEMPLATES = [
    {
        ...
        # Project-level templates root (ADR 0002 §3). App templates are
        # still found via APP_DIRS under apps/<app>/templates/.
        ...
        "OPTIONS": {
            "context_processors": [
                ...
                "apps.hub.context_processors.workspace_context",
            ],
        },
    },
]
...
# Project-level static sources (ADR 0002 §3). App static is still found
# via the AppDirectoriesFinder under apps/<app>/static/.
