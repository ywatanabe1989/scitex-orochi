...
def _post_dispatch_message(agent_name: str, workspace_id: int, text: str, metadata: dict) -> Optional[int]:
    ...
    try:
        ...
        from apps.hub.consumers import _sanitize_group
        from apps.hub.models import Message, Workspace, WorkspaceMember
        from apps.hub.views.api._dms import _ensure_dm_channel
    ...
    if _in_async_context():
        ...
        def _bg_hydrate():
            ...
            if ts is None:
                return
            try:
                from apps.hub.registry import _agents as _a
                from apps.hub.registry import _lock as _l
