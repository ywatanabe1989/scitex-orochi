// @ts-nocheck
import { _topoOpenChannelCompose } from "./compose";
import { renderActivityTab } from "./init";
import { setCurrentChannel } from "../app/state";
import { apiUrl, getCsrfToken, userName } from "../app/utils";
...
export var _topoClickState = null; /* {kind, name, count, timer, x, y} */
export var TOPO_CLICK_WINDOW_MS = 350;
export function _topoFlushClick() {
  ...
  if (s.count >= 3) {
    (globalThis as any)._overviewExpanded = (globalThis as any)._overviewExpanded === s.name ? null : s.name;
...
export function _topoBumpClick(name, clientX, clientY, kind) {
  var k = kind || "agent";
  if (
    _topoClickState &&
...
export function _openAgentDm(agentName) {
  if (!agentName) return;
  var human =
    (typeof userName !== "undefined" && userName) ||
    window.__orochiUserName ||
...
(globalThis as any)._overviewGridWired = (typeof _overviewGridWired !== 'undefined' ? _overviewGridWired : undefined);
