// @ts-nocheck
import { _openAgentDm, _topoClickState } from "./click";
import { _topoCleanupDrag, _topoClearDrop, _topoShowSubscribeToast, _topoSpawnGhost } from "./compose";
import { _topoSvgPoint } from "./grid-delegation";
import { renderActivityTab } from "./init";
import { _topoPoolSelectClear, _topoPoolSelectionHas } from "./multiselect";
import { _topoSetManualPosition, _topoStickyEdges, _topoStickyKey } from "./state";
import { _agentSubscribe, _showMiniToast } from "../app/agent-actions";
...
export function _ovgWireMouse(grid) {
  ...
  grid.addEventListener("mousedown", function (ev) {
    ...
    if (source === "pool" && _topoPoolSelectionHas(kind, name)) {
      items = [];
      Object.keys((globalThis as any)._topoPoolSelection.agents).forEach(function (n) {
        items.push({ kind: "agent", name: n });
      });
      Object.keys((globalThis as any)._topoPoolSelection.channels).forEach(function (n) {
        items.push({ kind: "channel", name: n });
      });
    }
    (globalThis as any)._topoDragState = {
      svg: svg,
      source: source,
      kind: kind,
      name: name,
  ...
  grid.addEventListener("mousemove", function (ev) {
    var s = (globalThis as any)._topoDragState;
    if (!s) return;
  ...
  grid.addEventListener("mouseup", function (ev) {
    ...
    if (s.moved && target) {
      ...
      if (subscribedAgentsOnCh + subscribedChsOnAg > 0) {
        ...
        (globalThis as any)._topoLastSig = ""; /* force re-render with new edges */
        renderActivityTab();
      }
    }
    ...
    if (s.moved) {
      ...
      setTimeout(function () {
        if ((globalThis as any)._topoDragState === s) (globalThis as any)._topoDragState = null;
      }, 0);
    } else {
      _topoCleanupDrag();
    }
