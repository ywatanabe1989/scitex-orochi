// @ts-nocheck
import { TOPO_SEEK_HEAT_BINS, TOPO_SEEK_HEAT_THROTTLE_MS, TOPO_SEEK_WINDOW_MS, _topoSeekEvents, _topoSeekSpeed } from "./state";
import { _topoPulseEdge } from "./topology-pulse";
...
export function _topoSeekbarHtml() {
  var mode = (globalThis as any)._topoSeekMode;
  var playing = (globalThis as any)._topoSeekPlaying;
...
export function _topoSeekHeatColor(intensity) {
...
export function _topoSeekHeatCompute(winStart, winEnd, bins) {
...
export function _topoSeekHeatPaint(canvas, force) {
  if (!canvas) return;
  var now =
    typeof performance !== "undefined" && performance.now
      ? performance.now()
      : Date.now();
  if (!force && now - (globalThis as any)._topoSeekHeatLastPaint < TOPO_SEEK_HEAT_THROTTLE_MS) {
    return;
  }
  ...
  if (!force && sig === (globalThis as any)._topoSeekHeatLastSig) return;
  (globalThis as any)._topoSeekHeatLastSig = sig;
  (globalThis as any)._topoSeekHeatLastPaint = now;
...
export function _topoSeekHeatRefresh(force) {
...
export function _topoSeekBuffer() {
...
export function _topoSeekFormatLabel(ts, mode) {
...
export function _topoSeekUpdateUI() {
  var bar = document.querySelector(".activity-view-topology .topo-seekbar");
  if (!bar) return;
  ...
  if ((globalThis as any)._topoSeekMode === "live") {
    ...
    if (range && !(globalThis as any)._topoSeekInteracting) range.value = "1000";
    ...
    if (range && !(globalThis as any)._topoSeekInteracting) {
      var permill = Math.round((((globalThis as any)._topoSeekTime - w.start) / span) * 1000);
      range.value = String(Math.max(0, Math.min(1000, permill)));
    }
    if (label)
      label.textContent = _topoSeekFormatLabel((globalThis as any)._topoSeekTime, "playback");
    if (live) live.classList.remove("topo-seek-live-on");
    bar.classList.add("topo-seek-playback");
  }
  if (play) {
    play.textContent = (globalThis as any)._topoSeekPlaying ? "❚❚" : "▶";
    play.title = (globalThis as any)._topoSeekPlaying ? "Pause" : "Play";
  }
...
export function _topoSeekEnterPlayback(ts) {
  (globalThis as any)._topoSeekMode = "playback";
  (globalThis as any)._topoSeekTime = ts;
}

export function _topoSeekEnterLive() {
  (globalThis as any)._topoSeekMode = "live";
  _topoSeekStopPlay();
  _topoSeekUpdateUI();
}

export function _topoSeekReplayOne(ev) {
  ...
  (globalThis as any)._topoSeekReplayInProgress = true;
  try {
    _topoPulseEdge(ev.sender, ev.channel, ev.opts);
  } finally {
    (globalThis as any)._topoSeekReplayInProgress = false;
  }
}

export function _topoSeekStartPlay() {
  if ((globalThis as any)._topoSeekPlaying) return;
  ...
  (globalThis as any)._topoSeekPlaying = true;
  (globalThis as any)._topoSeekLastFrameTs = 0;
  function _tick(now) {
    if (!(globalThis as any)._topoSeekPlaying) return;
    if (!(globalThis as any)._topoSeekLastFrameTs) (globalThis as any)._topoSeekLastFrameTs = now;
    var dt = (now - (globalThis as any)._topoSeekLastFrameTs) * _topoSeekSpeed;
    (globalThis as any)._topoSeekLastFrameTs = now;
    var prev = (globalThis as any)._topoSeekTime;
    (globalThis as any)._topoSeekTime += dt;
    var w = _topoSeekBuffer();
    /* Fire every event that crossed the playhead in this frame. */
    for (var i = 0; i < _topoSeekEvents.length; i++) {
      var ev = _topoSeekEvents[i];
      if (ev.ts > prev && ev.ts <= (globalThis as any)._topoSeekTime) {
        _topoSeekReplayOne(ev);
      }
    }
    if ((globalThis as any)._topoSeekTime >= w.end) {
    ...
    _topoSeekUpdateUI();
    (globalThis as any)._topoSeekRafId = requestAnimationFrame(_tick);
  }
  (globalThis as any)._topoSeekRafId = requestAnimationFrame(_tick);
  _topoSeekUpdateUI();
}

export function _topoSeekStopPlay() {
  (globalThis as any)._topoSeekPlaying = false;
  if ((globalThis as any)._topoSeekRafId != null) {
    cancelAnimationFrame((globalThis as any)._topoSeekRafId);
    (globalThis as any)._topoSeekRafId = null;
  }
  _topoSeekUpdateUI();
}

export function _topoSeekTogglePlay() {
  if ((globalThis as any)._topoSeekPlaying) _topoSeekStopPlay();
  else _topoSeekStartPlay();
}

export function _wireTopoSeekbar(grid) {
  if ((globalThis as any)._topoSeekWired || !grid) return;
  (globalThis as any)._topoSeekWired = true;
  ...
  grid.addEventListener("input", function (ev) {
    ...
    (globalThis as any)._topoSeekInteracting = true;
    var w = _topoSeekBuffer();
  ...
  grid.addEventListener("change", function (ev) {
    var t = ev.target;
    if (!t || t.getAttribute("data-topo-seek") !== "range") return;
    (globalThis as any)._topoSeekInteracting = false;
  });
   ...
   * idempotent-guarded by (globalThis as any)._topoSeekWired. */
