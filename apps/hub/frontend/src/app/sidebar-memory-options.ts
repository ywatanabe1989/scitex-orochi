// @ts-nocheck
import { _topoPoolMemoryIsDirty, _topoPoolSelectionSize } from "../activity-tab/multiselect";
 ...
 * label. */
export function _buildSidebarMemoryDropdownOptions() {
  var max = typeof _TOPO_POOL_MEM_MAX !== "undefined" ? _TOPO_POOL_MEM_MAX : 5;
  var active = (globalThis as any)._topoActiveMemSlot;
  var parts = [];
...
export function _buildSidebarMemoryChipsHtml() {
  var html = "";
  var max = typeof _TOPO_POOL_MEM_MAX !== "undefined" ? _TOPO_POOL_MEM_MAX : 5;
  for (var slot = 1; slot <= max; slot++) {
    var mem = _topoPoolMemories ? _topoPoolMemories[String(slot)] : null;
    ...
    var label = mem && mem.label ? String(mem.label) : "";
    var active = (globalThis as any)._topoActiveMemSlot === slot;
    var dirty =
      active &&
      typeof _topoPoolMemoryIsDirty === "function" &&
      _topoPoolMemoryIsDirty(slot);
 ...
 * the same (globalThis as any)._topoActiveMemSlot + selection-size inputs. */
export function _updateMemorySaveBtn(saveBtn) {
  if (!saveBtn) return;
  var hasSel =
  ...
  saveBtn.classList.toggle(
    ...
    hasSel || (globalThis as any)._topoActiveMemSlot != null,
  );
  ...
  saveBtn.textContent =
    (globalThis as any)._topoActiveMemSlot != null ? "Save to M" + (globalThis as any)._topoActiveMemSlot : "Save";
}
 ...
 * nodes become no-ops. */
export function renderSidebarMemory() {
