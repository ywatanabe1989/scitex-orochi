...
import { _syncMemoryDirtyIndicators, _topoPersistActiveMemSlot, _topoPoolMemoryIsDirty, _topoPoolMemoryRecall, _topoPoolMemoryRename, _topoPoolMemorySave, _topoPoolSelectClear, _topoPoolSelectionPaint } from "../activity-tab/multiselect";
import { _TOPO_POOL_MEM_MAX, _topoPoolMemories } from "../activity-tab/state";
import { renderSidebarMemory } from "./sidebar-memory-options";
...
export function _sidebarMemoryShowHint(anchor, text) {
  if (!anchor) return;
...
export function _sidebarMemoryRefreshBothSurfaces() {
  renderSidebarMemory();
  if (typeof (globalThis as any)._topoLastSig !== "undefined") (globalThis as any)._topoLastSig = "";
...
export function _memSelectOnChange(selectEl) {
  if (!selectEl) return;
  var val = selectEl.value;
  var prevSlot = (globalThis as any)._topoActiveMemSlot;
  var prevDirty =
    prevSlot != null &&
  ...
  if (val === "") {
    (globalThis as any)._topoActiveMemSlot = null;
    ...
    (globalThis as any)._topoActiveMemSlot = free;
    if (typeof _topoPoolSelectClear === "function") {
      _topoPoolSelectClear();
    }
    if (
...
export function _memSaveActionHandler(anchor) {
  if ((globalThis as any)._topoActiveMemSlot == null) {
    _sidebarMemoryShowHint(anchor, "Pick an M-slot first");
    return false;
  }
  if (typeof _topoPoolMemorySave === "function") {
    _topoPoolMemorySave((globalThis as any)._topoActiveMemSlot);
  }
  return true;
}
