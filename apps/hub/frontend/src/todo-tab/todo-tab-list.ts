...
import { PRIORITY_GROUPS, _backgroundFillDetails, attachTodoEvents, buildGroupHtml, classifyIssue, sortByUpdated } from "./todo-tab-helpers";
import { renderVizTab } from "../viz-tab";
 ...
 *          (globalThis as any)._todoIssuesByNumber */

/* Active filter groups — Set so Ctrl/Cmd-click can multi-select */
export var todoActiveGroups = new Set(["all"]);

export function _syncTodoBtnState() {
...
export function _updateTodoBtnCounts(issues) {
  var counts = {
    all: 0,
    "high-priority": 0,
    "medium-priority": 0,
...
export var _TODO_VIEW_KEY = "orochi.todoViewMode";
export function _todoViewMode() {
  try {
...
export function _applyTodoViewMode(mode) {
  var isViz = mode === "viz";
...
export function _wireTodoViewSwitch() {
  document
    .querySelectorAll("#todo-view [data-todo-view]")
    .forEach(function (btn) {
...
export function _todoBackendState() {
...
export var _todoCache = { all: null, open: null };
export var _todoCacheTs = { all: 0, open: 0 };
export var _TODO_CACHE_TTL_MS = 60 * 1000; // refetch at most once/min on pill clicks

export function _hasBlockerLabel(issue) {
  return (issue.labels || []).some(function (l) {
    return (l.name || "").toLowerCase() === "blocker";
  });
}

export function _passesGroupFilter(issue) {
...
export function _populateIssueMap(issues) {
  (globalThis as any)._todoIssuesByNumber = {};
  (issues || []).forEach(function (i) {
    (globalThis as any)._todoIssuesByNumber[i.number] = i;
  });
}

export function _updateLastFetchedLabel(ts) {
...
export function _renderTodoFromCache(issues) {
...
export async function fetchTodoList(forceRefresh) {
  var msgInput = document.getElementById("msg-input");
  var inputHasFocus = msgInput && document.activeElement === msgInput;
  var savedStart = inputHasFocus ? msgInput.selectionStart : 0;
  var savedEnd = inputHasFocus ? msgInput.selectionEnd : 0;
