// @ts-nocheck
import { apiUrl, messageKey } from "../app/utils";
import { appendMessage } from "./chat-render";
import { fetchReactionsForMessages } from "../reactions";
import { applyThreadUrlOnLoad } from "../threads/state";
import { _renderAttachmentTray } from "../upload";
...
export async function loadHistory() {
  ...
  if ((globalThis as any).currentChannel) {
    return loadChannelHistory((globalThis as any).currentChannel);
  }
  try {
    ...
    var savedAttachments =
      typeof (globalThis as any).pendingAttachments !== "undefined"
        ? (globalThis as any).pendingAttachments.slice()
        : [];

    container.innerHTML = "";
    (globalThis as any).knownMessageKeys = {};
    /* Suppress arrival animations / sidebar pulses for this bulk backlog. */
    (globalThis as any)._isLoadingHistory = true;
    try {
      ...
      (globalThis as any)._isLoadingHistory = false;
      (globalThis as any)._initialLoadComplete = true;
    }
    ...
    if (
      ...
      typeof (globalThis as any).pendingAttachments !== "undefined" &&
      !(globalThis as any).pendingAttachments.length
    ) {
      (globalThis as any).pendingAttachments = savedAttachments;
      if (typeof _renderAttachmentTray === "function") _renderAttachmentTray();
    }

    (globalThis as any).historyLoaded = true;
...
export async function fetchNewMessages() {
  try {
    var endpoint = (globalThis as any).currentChannel
      ? "/api/history/" + encodeURIComponent((globalThis as any).currentChannel) + "?limit=50"
    ...
    try {
      messages.forEach(function (row) {
        ...
        appendMessage({
          ...
          metadata: row.metadata || {},
          payload: {
            channel: row.channel || (globalThis as any).currentChannel || "",
            content: row.content,
            attachments:
              (row.metadata && row.metadata.attachments) ||
              row.attachments ||
...
export async function loadChannelHistory(channel) {
  try {
