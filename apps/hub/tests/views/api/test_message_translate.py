...
from apps.hub.models import Channel, Message, Workspace, WorkspaceMember
...
class MessageTranslateTest(TestCase):
    ...
    def test_no_api_key_returns_503(self):
        with patch.dict("os.environ", {}, clear=True):
            import importlib
            import apps.hub.views.api._translate as m
            importlib.reload(m)
    ...
    def test_404_for_missing_message(self):
        ...
        with patch("apps.hub.views.api._translate.httpx.post", return_value=mock_resp):
    ...
    def test_api_error_returns_502(self):
        ...
        with patch("apps.hub.views.api._translate.httpx.post", side_effect=exc):
            with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "sk-test"}):
                resp = self._post(self.msg.id)
