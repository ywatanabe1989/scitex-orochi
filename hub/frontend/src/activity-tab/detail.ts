// @ts-nocheck
import { _bindActivityChannelControls } from "./channel-controls";
import { _activityPaneSshState, _bindActivityPaneControls, _bindActivitySendInput } from "./detail-bind";
import { _renderHookPanels } from "./hooks-panel";
import { ACTIVITY_FOLLOW_INTERVAL_MS, _activityDetailCache, _activityPaneExpanded, _colorKeyFor } from "./state";
import { _secondsSinceIso, _stripAnsi } from "./utils";
import { cleanAgentName, escapeHtml, getAgentColor } from "../app/utils";

/* activity-tab/detail.js — per-agent full detail view renderer
 * (_renderActivityAgentDetail). */

/* Per-agent full detail view */
export function _renderActivityAgentDetail(a, grid) {
  ...
  var _isFollowing = (globalThis as any)._activityFollowAgent === a.name;
