// @ts-nocheck
import { _topoSelectedNames } from "./multiselect";
import { _topoHiddenSignature, _topoManualPositionsSignature, _topoStickyEdges } from "./state";
...
export function _topoSignature(visible) {
  /* Digest: color-key selection + multi-select set + per-agent (name +
   * online-ness + liveness bucket + pinned + channel count).
   * (globalThis as any)._overviewColor goes in because swapping "color: host / account"
  ...
  var parts = [
    (globalThis as any)._overviewColor || "name",
    "sel:" + selSig,
    "prefs:" + prefSig,
    "sticky:" + stickySig,
