...
import { _markerAttrsForPerm } from "./topology-arrows";
...
export function _topoBuildEdgesHtml(visible, agentPos, chPos, chSet, humanName) {
  ...
  visible.forEach(function (a) {
    ...
    (a.channels || []).forEach(function (c) {
      var cp = chPos[c];
      if (!ap || !cp) return;
      var perm = (globalThis as any)._topoChannelPerms[_permKey(c, a.name)] || "read-write";
