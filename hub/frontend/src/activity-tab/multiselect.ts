// @ts-nocheck
import { _TOPO_ACTIVE_MEM_KEY, _TOPO_POOL_MEM_MAX, _TOPO_POOL_SEL_KEY, _topoMemWorkspaceKey, _topoPoolMemories, _topoSaveHidden } from "./state";
import { activeTags, renderTags, syncFilterVisuals } from "../filter/state";
...
export var _topoSelected = Object.create(null); /* name → true */
export function _topoSelectedNames() {
  return Object.keys(_topoSelected);
}
export function _topoSelectClear() {
  _topoSelected = Object.create(null);
}
export function _topoSelectToggle(name) {
  if (_topoSelected[name]) delete _topoSelected[name];
  else _topoSelected[name] = true;
}
export function _topoSelectAdd(name) {
  if (name) _topoSelected[name] = true;
}

export function _topoPersistActiveMemSlot() {
  try {
    if ((globalThis as any)._topoActiveMemSlot == null)
      localStorage.removeItem(_TOPO_ACTIVE_MEM_KEY);
    else localStorage.setItem(_TOPO_ACTIVE_MEM_KEY, String((globalThis as any)._topoActiveMemSlot));
  } catch (_) {}
}
...
export function _topoAutoSaveActiveSlot() {
...
/* Dirty-state check: compare the current (globalThis as any)._topoPoolSelection to the
...
export function _topoPoolMemoryIsDirty(slot) {
  ...
  var liveA = (globalThis as any)._topoPoolSelection.agents || {};
  var liveC = (globalThis as any)._topoPoolSelection.channels || {};
...
export function _topoPoolPersistSelection() {
  try {
    localStorage.setItem(
      _TOPO_POOL_SEL_KEY,
      JSON.stringify({
        agents: Object.keys((globalThis as any)._topoPoolSelection.agents),
        channels: Object.keys((globalThis as any)._topoPoolSelection.channels),
      }),
    );
  } catch (_e) {
...
export function _topoPoolPersistMemories() {
  try {
    localStorage.setItem(
...
export function _topoPoolSelectionSize() {
  return (
    Object.keys((globalThis as any)._topoPoolSelection.agents).length +
    Object.keys((globalThis as any)._topoPoolSelection.channels).length
  );
}
export function _topoPoolSelectionHas(kind, name) {
  var bucket = kind === "channel" ? "channels" : "agents";
  return !!(globalThis as any)._topoPoolSelection[bucket][name];
}
export function _topoPoolSelectToggle(kind, name) {
  var bucket = kind === "channel" ? "channels" : "agents";
  if ((globalThis as any)._topoPoolSelection[bucket][name]) delete (globalThis as any)._topoPoolSelection[bucket][name];
  else (globalThis as any)._topoPoolSelection[bucket][name] = true;
  _topoPoolPersistSelection();
}
export function _topoPoolSelectClear() {
  (globalThis as any)._topoPoolSelection = {
    agents: Object.create(null),
    channels: Object.create(null),
  };
  _topoPoolPersistSelection();
}
export function _topoPoolSelectOnly(kind, name) {
  (globalThis as any)._topoPoolSelection = {
    agents: Object.create(null),
    channels: Object.create(null),
  };
  var bucket = kind === "channel" ? "channels" : "agents";
  (globalThis as any)._topoPoolSelection[bucket][name] = true;
  _topoPoolPersistSelection();
}
...
export function _topoPoolSelectAll(grid) {
  ...
  for (var i = 0; i < chips.length; i++) {
    ...
    if (ag) (globalThis as any)._topoPoolSelection.agents[ag] = true;
    else if (ch) (globalThis as any)._topoPoolSelection.channels[ch] = true;
  }
  _topoPoolPersistSelection();
}
...
export function _topoPoolSelectRange(targetChip) {
  ...
  for (var j = lo; j <= hi; j++) {
    ...
    if (ag2) (globalThis as any)._topoPoolSelection.agents[ag2] = true;
    else if (c2) (globalThis as any)._topoPoolSelection.channels[c2] = true;
  }
  _topoPoolPersistSelection();
}
...
export function _topoPoolMemorySnapshot() {
  ...
  return {
    ...
    channels: Object.keys((globalThis as any)._topoPoolSelection.channels),
    hidden: {
      agents: Object.keys((globalThis as any)._topoHidden.agents || {}),
      channels: Object.keys((globalThis as any)._topoHidden.channels || {}),
    },
    filter: { input: rawInput, tags: tags },
    savedAt: Date.now(),
  };
}
export function _topoPoolMemorySave(slot) {
...
export function _topoPoolMemoryRename(slot, label) {
  var mem = _topoPoolMemories[String(slot)];
  if (!mem) return false;
...
export function _topoPoolMemoryRecall(slot) {
  var mem = _topoPoolMemories[String(slot)];
  if (!mem) return false;
  /* Pool selection. */
  (globalThis as any)._topoPoolSelection = {
  ...
  (mem.agents || []).forEach(function (n) {
    (globalThis as any)._topoPoolSelection.agents[n] = true;
  });
  (mem.channels || []).forEach(function (n) {
    (globalThis as any)._topoPoolSelection.channels[n] = true;
  });
  _topoPoolPersistSelection();
  ...
  if (mem.hidden && typeof mem.hidden === "object") {
    (globalThis as any)._topoHidden = { agents: {}, channels: {} };
    (mem.hidden.agents || []).forEach(function (n) {
      (globalThis as any)._topoHidden.agents[n] = true;
    });
    (mem.hidden.channels || []).forEach(function (n) {
      (globalThis as any)._topoHidden.channels[n] = true;
    });
    _topoSaveHidden();
...
export function _topoPoolMemoryDelete(slot) {
  if (!_topoPoolMemories[String(slot)]) return;
  delete _topoPoolMemories[String(slot)];
  _topoPoolPersistMemories();
}
export function _topoPoolMemoryNextFreeSlot() {
 ...
 * as filters". */
export function _topoPoolSelectionPaint(root) {
  ...
  for (var i = 0; i < chips.length; i++) {
    ...
    var ch = chip.getAttribute("data-channel");
    var sel = ag
      ? !!(globalThis as any)._topoPoolSelection.agents[ag]
      : !!(ch && (globalThis as any)._topoPoolSelection.channels[ch]);
  ...
  for (var m = 0; m < memBtns.length; m++) {
    ...
    memBtns[m].classList.toggle(
      ...
      (globalThis as any)._topoActiveMemSlot === slotN,
    );
    ...
    var dirty = (globalThis as any)._topoActiveMemSlot === slotN && _topoPoolMemoryIsDirty(slotN);
 ...
 * its own renders. */
export function _syncMemoryDirtyIndicators() {
...
export function _topoPoolApplyCanvasFilter(root) {
  var host = root || document;
  var svg = host.querySelector(".topo-svg");
  if (!svg) return;
  var selA = (globalThis as any)._topoPoolSelection.agents;
  var selC = (globalThis as any)._topoPoolSelection.channels;
...
(globalThis as any)._topoPoolSelAnchor = (typeof _topoPoolSelAnchor !== 'undefined' ? _topoPoolSelAnchor : undefined);
