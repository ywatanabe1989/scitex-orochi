...
import { agentIdentity, channelIdentity } from "../agent-icons";
import { escapeHtml, getAgentColor } from "../app/utils";
...
export function _topoBuildPoolHtml(visible, channels) {
...
export function _topoBuildPoolHtmlLegacy(visible, channels) {
  ...
  var poolAgentsHtml = visible
    ...
    .map(function (a) {
      var selCls = (globalThis as any)._topoPoolSelection.agents[a.name]
  ...
  var poolChannelsHtml = Object.keys(poolChSet)
    .sort()
    .map(function (c) {
      var selCls = (globalThis as any)._topoPoolSelection.channels[c]
        ? " topo-pool-chip-selected"
        : "";
