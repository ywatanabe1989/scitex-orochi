...
import { TOPO_PERMS_TTL_MS, _activityDetailCache, _activityDetailInflight, _topoChannelPermsInflight } from "./state";
import { _repaintTopoArrows } from "./topology-arrows";
import { apiUrl } from "../app/utils";

/* activity-tab/data.js — data fetch + refresh polling + channel-member
 * permission fetch. */

export async function _fetchActivityDetail(name) {
  ...
  try {
    ...
    if ((globalThis as any)._overviewExpanded === name) {
...
export async function _activityChannelRequest(method, agent, channel) {
...
export function _invalidateTopoPerms() {
  (globalThis as any)._topoChannelPerms = Object.create(null);
  (globalThis as any)._topoChannelPermsFetchedAt = 0;
}
window._invalidateTopoPerms = _invalidateTopoPerms;


export function _permKey(channel, agentName) {
  return channel + "::" + agentName;
}

/* Fetch membership+permission for one channel and fold the result into
 * (globalThis as any)._topoChannelPerms. Silent on failure — the caller treats missing
 * entries as read-write. */
export async function _fetchTopoPermsForChannel(channel) {
  ...
  try {
    ...
    rows.forEach(function (row) {
      ...
      (globalThis as any)._topoChannelPerms[_permKey(channel, bare)] = perm;
      (globalThis as any)._topoChannelPerms[_permKey(channel, uname)] = perm;
    });
  } catch (_e) {
...
export function _refreshTopoPerms(channels) {
  var now = Date.now();
  if (now - (globalThis as any)._topoChannelPermsFetchedAt < TOPO_PERMS_TTL_MS) return;
  (globalThis as any)._topoChannelPermsFetchedAt = now;
...
export async function refreshActivityFromApi() {
  try {
...
export function startActivityAutoRefresh() {
  if ((globalThis as any).activityRefreshTimer) return;
  ...
  (globalThis as any).activityRefreshTimer = setInterval(refreshActivityFromApi, 30000);
}

export function stopActivityAutoRefresh() {
  if ((globalThis as any).activityRefreshTimer) {
    clearInterval((globalThis as any).activityRefreshTimer);
    (globalThis as any).activityRefreshTimer = null;
  }
}
