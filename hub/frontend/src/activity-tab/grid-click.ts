// @ts-nocheck
import { _topoBumpClick } from "./click";
import { _openTopoGroupCompose } from "./compose";
import { _topoShowEdgeMenu } from "./edge-menu";
import { renderActivityTab } from "./init";
import { _topoPoolSelectOnly, _topoPoolSelectRange, _topoPoolSelectToggle, _topoPoolSelectionPaint, _topoSelectClear, _topoSelectToggle, _topoSelectedNames } from "./multiselect";
import { _topoUnhide } from "./state";
import { openAvatarPicker, openHumanAvatarPicker } from "../agent-icons";
import { _setChannelPref } from "../app/channel-prefs";
import { togglePinAgent } from "../app/sidebar-agents";
import { _memSaveActionHandler, _sidebarMemoryRefreshBothSurfaces } from "../app/sidebar-memory-handlers";
...
export function _ovgWireClick(grid) {
  grid.addEventListener("click", function (ev) {
    ...
    if (card && grid.contains(card)) {
      ...
      (globalThis as any)._overviewExpanded = (globalThis as any)._overviewExpanded === cname ? null : cname;
      renderActivityTab();
      return;
    }
    ...
    if (poolChip && grid.contains(poolChip)) {
      if ((globalThis as any)._topoDragState && (globalThis as any)._topoDragState.suppressClick) {
        (globalThis as any)._topoDragState.suppressClick = false;
        ev.stopPropagation();
        return;
      }
      ...
      var prefHidden =
        pcKind === "channel" && (window._channelPrefs || {})[pcName]
          ...
          : false;
      var isHidden =
        (pcKind === "agent" && (globalThis as any)._topoHidden.agents[pcName]) ||
        (pcKind === "channel" && (globalThis as any)._topoHidden.channels[pcName]) ||
        prefHidden;
      ...
      if (ev.shiftKey && !ev.altKey) {
        ...
        (globalThis as any)._topoPoolSelAnchor = { kind: pcKind, name: pcName };
      } else if (ev.altKey) {
