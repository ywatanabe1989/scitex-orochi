...
import { ACTIVITY_FOLLOW_INTERVAL_MS, _activityDetailCache, _activityPaneExpanded, _paneShowRaw } from "./state";
import { _stripAnsi } from "./utils";
...
export function _bindActivitySendInput(grid, name) {
...
export function _bindActivityPaneControls(grid, name, pane, paneFull) {
  grid
  ...
  grid
    ...
    .forEach(function (btn) {
      btn.addEventListener("click", function (ev) {
        ev.preventDefault();
        if ((globalThis as any)._activityFollowAgent === name) {
          _stopActivityFollow();
        } else {
...
export function _activityPaneCloseSsh(name) {
...
export function _stopActivityFollow() {
  if ((globalThis as any)._activityFollowTimer != null) {
    clearInterval((globalThis as any)._activityFollowTimer);
    (globalThis as any)._activityFollowTimer = null;
  }
  (globalThis as any)._activityFollowAgent = null;
  document
    .querySelectorAll('[data-act-pane-action="follow"]')
    .forEach(function (b) {
...
export function _startActivityFollow(name) {
  _stopActivityFollow();
  (globalThis as any)._activityFollowAgent = name;
  document
    .querySelectorAll(
  ...
  (globalThis as any)._activityFollowTimer = setInterval(function () {
    if (!(globalThis as any)._activityFollowAgent) return;
    if (typeof document !== "undefined" && document.hidden) return;
    if ((globalThis as any)._overviewExpanded !== (globalThis as any)._activityFollowAgent) {
      _stopActivityFollow();
      return;
    }
    delete _activityDetailCache[(globalThis as any)._activityFollowAgent];
    _fetchActivityDetail((globalThis as any)._activityFollowAgent);
