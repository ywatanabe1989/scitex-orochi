// @ts-nocheck
import { _invalidateTopoPerms } from "./data";
import { renderActivityTab } from "./init";
import { _topoStickyEdges, _topoStickyKey } from "./state";
import { _agentDjangoUsername, _showMiniToast } from "../app/agent-actions";
import { fetchAgents } from "../app/sidebar-agents";
import { apiUrl, escapeHtml, getAgentColor, getCsrfToken } from "../app/utils";
import { fetchAgentsThrottled } from "../app/websocket";
...
export var _topoEdgeMenuEl = null;
export function _topoCloseEdgeMenu() {
...
export function _topoClampMenuInViewport(el) {
  if (!el) return;
  var pad = 8;
...
export function _topoEdgeMenuOutsideClick(ev) {
  if (!_topoEdgeMenuEl) return;
  if (_topoEdgeMenuEl.contains(ev.target)) return;
  _topoCloseEdgeMenu();
}
export function _topoEdgeMenuKeyHandler(ev) {
  if (ev.key === "Escape") {
    ev.stopPropagation();
    _topoCloseEdgeMenu();
  }
}
export function _topoDoEdgeUnsubscribe(agent, channel) {
...
export function _topoShowEdgeMenu(agent, channel, clientX, clientY) {
  _topoCloseEdgeMenu();
