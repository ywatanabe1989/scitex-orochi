// @ts-nocheck
import { apiUrl, orochiHeaders, userName } from "./app/utils";

/* Reactions — emoji reactions on messages */
/* globals: apiUrl, orochiHeaders, userName */

export var COMMON_EMOJI = ["\uD83D\uDC4D", "\u2764\uFE0F", "\uD83D\uDC40", "\uD83D\uDE04", "\uD83C\uDF89", "\uD83E\uDD14", "\u2705", "\u274C"];
/* 👍 ❤️ 👀 😄 🎉 🤔 ✅ ❌ */

export var reactionPicker = null;
...
export function openReactionPicker(btn, messageId) {
  closeReactionPicker();
...
export async function fetchReactionsForMessage(messageId) {
  try {
...
export function renderReactionsForMessage(messageId, reactionsByEmoji) {
  var container = document.querySelector('.msg-reactions[data-msg-id="' + messageId + '"]');
  if (!container) return;
...
export function handleReactionUpdate(msg) {
