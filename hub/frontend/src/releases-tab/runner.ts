...
import { REPOS, _getCsrf, changelogCache, fetchTrackedRepos, persistOrder, renderMarkdown } from "./state";
 ...
 *
 * Companion file: releases-tab/state.js (loaded first) holds the in-
 * memory state (REPOS, changelogCache, (globalThis as any).activeRepoKey), the markdown
 * renderer, and the tracked-repos fetch/reorder helpers.
 */
/* globals: apiUrl, escapeHtml, getCookie,
 *          REPOS, changelogCache, (globalThis as any).releasesInitialized, (globalThis as any).activeRepoKey,
 *          renderMarkdown, _getCsrf, fetchTrackedRepos, persistOrder */
...
export function renderSubtabs() {
  ...
  var tabsHtml = REPOS.map(function (r) {
    ...
    var cls =
      ...
      (key === (globalThis as any).activeRepoKey ? " active" : "");
 ...
 * -------------------------------------------------------------------- */
export var _dragSrcId = null;

export function _clearDragIndicators() {
  var bar = document.getElementById("releases-subtabs");
  if (!bar) return;
...
export function attachDragHandlers(wrap) {
...
export function openAddRepoDialog() {
    ...
    .then(function (r) {
      ...
      return fetchTrackedRepos().then(function () {
        if (r.data && r.data.repo) {
          (globalThis as any).activeRepoKey = r.data.repo.key;
        }
        renderSubtabs();
        if ((globalThis as any).activeRepoKey) loadChangelog((globalThis as any).activeRepoKey);
      });
    })
    .catch(function (e) {
...
export function deleteRepo(id, key) {
  if (!id) return;
  ...
  fetch(apiUrl("/api/tracked-repos/" + id + "/"), {
    ...
    .then(function (r) {
      ...
      if ((globalThis as any).activeRepoKey === key) (globalThis as any).activeRepoKey = null;
      return fetchTrackedRepos().then(function () {
        if (!(globalThis as any).activeRepoKey && REPOS.length) {
          (globalThis as any).activeRepoKey = REPOS[0].owner + "/" + REPOS[0].repo;
        }
        renderSubtabs();
        var content = document.getElementById("releases-content");
        if ((globalThis as any).activeRepoKey) {
          loadChangelog((globalThis as any).activeRepoKey);
        } else if (content) {
          content.innerHTML =
...
export function selectRepo(key) {
  (globalThis as any).activeRepoKey = key;
  var bar = document.getElementById("releases-subtabs");
  if (bar) {
    bar.querySelectorAll(".releases-subtab-btn").forEach(function (btn) {
...
export function loadChangelog(key) {
  var msgInput = document.getElementById("msg-input");
  var inputHasFocus = msgInput && document.activeElement === msgInput;
  var savedStart = inputHasFocus ? msgInput.selectionStart : 0;
  var savedEnd = inputHasFocus ? msgInput.selectionEnd : 0;
  ...
  fetch(apiUrl("/api/repo/" + owner + "/" + repo + "/changelog/"), {
    ...
    .then(function (r) {
      if (key !== (globalThis as any).activeRepoKey) return; // user switched away
    ...
    .catch(function (e) {
      if (key !== (globalThis as any).activeRepoKey) return;
      var _mi = document.getElementById("msg-input");
      var _ihf = _mi && document.activeElement === _mi;
      var _ss = _ihf ? _mi.selectionStart : 0;
      var _se = _ihf ? _mi.selectionEnd : 0;
...
export function initReleasesTab() {
  if ((globalThis as any).releasesInitialized) return;
  (globalThis as any).releasesInitialized = true;
