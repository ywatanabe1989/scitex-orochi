 ...
 * perceptibly slow). */
export var _vizCachedData = null;
export var _vizCachedAt = 0;
export var _VIZ_FRESH_MS = 60_000;
...
export var _vizRenderedSig = "";

export function _vizSig(data) {
  try {
...
export function _buildVizSkeleton() {
...
export async function fetchVizPayload() {
  var container = document.getElementById("viz-content");
  if (!container) return;
  try {
...
export function _renderVizPayload(data, container) {
...
export function _buildLineChartSVG(series) {
  var W = 1200;
  var H = 360;
  var padL = 54;
  var padR = 20;
