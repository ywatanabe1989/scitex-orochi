...
import { apiUrl, cleanAgentName, csrfToken, escapeHtml, getAgentColor, hostedAgentName } from "./app/utils";

/* Agent icon system — Gravatar-style cascade: image > emoji > text > default SVG */
/* globals: escapeHtml */

export var cachedAgentIcons = {}; /* {name: image|emoji|text string} */
export var cachedAgentColors = {}; /* {name: hex color string} */
...
export var cachedHumanIcons = {};
export var cachedHumanColors = {};
 ...
 * leading '#'). */
export var cachedChannelIcons = {}; /* {"#general": image|emoji|text} */
export var cachedChannelColors = {}; /* {"#general": "#hex"} */

/* Inline SVG icon generators for branding — official SciTeX snake */
export function getSnakeIcon(size, color) {
  size = size || 20;
  color = color || "#4ecdc4";
  return (
...
export function getPersonIcon(size, color) {
  size = size || 20;
  color = color || "#c4a6e8";
  return (
    '<svg class="orochi-icon" width="' +
...
export function getSenderIcon(senderName, isAgent, size) {
  size = size || 18;
...
export var _avatarFileInput = null;
export var _avatarTargetAgent = null;

export function _ensureAvatarInput() {
...
export function openAvatarPicker(agentName) {
  _ensureAvatarInput();
  _avatarTargetAgent = agentName;
  _avatarFileInput.click();
}

export function uploadAgentAvatar(agentName, file) {
  var fd = new FormData();
...
export function fetchHumanProfiles() {
...
export var _humanAvatarFileInput = null;

export function _ensureHumanAvatarInput() {
...
export function openHumanAvatarPicker() {
  _ensureHumanAvatarInput();
  _humanAvatarFileInput.click();
}

export function uploadHumanAvatar(file) {
  var fd = new FormData();
  fd.append("file", file);
  var headers = {};
  if (typeof csrfToken !== "undefined" && csrfToken) {
...
export function openHumanAvatarTextEditor() {
...
export function renderMyAvatarButton() {
...
export function channelIdentity(ch) {
...
export function cacheChannelIdentity(ch) {
