...
export function _processMessageMarkdown(content) {
   ...
   * name to be sourced from (globalThis as any).cachedAgentNames/cachedMemberNames and by
   * using word boundaries so substrings inside other words don't match. */
  function _highlightBareNames(s) {
    var names = [];
    if (
      typeof (globalThis as any).cachedAgentNames !== "undefined" &&
      Array.isArray((globalThis as any).cachedAgentNames)
    ) {
      names = names.concat((globalThis as any).cachedAgentNames);
    }
    if (
