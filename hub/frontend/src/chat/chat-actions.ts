...
import { apiUrl, cleanAgentName, escapeHtml, orochiHeaders, timeAgo, userName } from "../app/utils";
import { loadChannelHistory, loadHistory } from "./chat-history";
import { addTag } from "../filter/state";
import { openReactionPicker } from "../reactions";
...
export function saveEditMessage(msgId, newText) {
...
export function handleMessageEdit(event) {
  var msgInput = document.getElementById("msg-input");
  var inputHasFocus = msgInput && document.activeElement === msgInput;
  var savedStart = inputHasFocus ? msgInput.selectionStart : 0;
  var savedEnd = inputHasFocus ? msgInput.selectionEnd : 0;
...
export function handleMessageDelete(event) {
  var el = document.querySelector(
    '.msg[data-msg-id="' + event.message_id + '"]',
  );
  if (el) {
