// @ts-nocheck
import { _renderAgentContent } from "./controls";
...
export var _agentDetailCache = {}; /* name -> last /detail response */
export var _agentDetailInflight = {}; /* name -> bool (in-flight guard) */
...
export async function _fetchAgentDetail(name) {
...
export function _invalidateAgentDetail(name) {
...
export function onAgentInfoEvent(name) {
  if (!name) return;
...
export function livenessColor(liveness) {
  switch (liveness) {
    case "online":
      return "#4ecdc4";
    case "idle":
...
export function formatUptime(isoStr) {
  if (!isoStr) return "-";
...
(globalThis as any)._agentsTabInterval = (typeof _agentsTabInterval !== 'undefined' ? _agentsTabInterval : undefined);
(globalThis as any)._followAgent = (typeof _followAgent !== 'undefined' ? _followAgent : undefined);
(globalThis as any)._followTimer = (typeof _followTimer !== 'undefined' ? _followTimer : undefined);
(globalThis as any)._lastAgentsData = (typeof _lastAgentsData !== 'undefined' ? _lastAgentsData : undefined);
(globalThis as any)._selectedAgentTab = (typeof _selectedAgentTab !== 'undefined' ? _selectedAgentTab : undefined);
