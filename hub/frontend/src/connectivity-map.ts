...
import { hideMachineTooltip, moveMachineTooltip, showMachineTooltip } from "./resources-tab/panel";

/* Connectivity map — SSH mesh visualization in Machines tab */
/* globals: apiUrl, escapeHtml */

export var connectivityCache = null;
 ...
 * require. */
export function syncHostHover(host, on) {
  if (!host) return;
  var selectors = [
...
export function _layoutNodes(nodes, cx, cy, innerRadius, outerRadius) {
  var positions = {};
...
export function _edgePath(p1, p2, offsetSign) {
  var dx = p2.x - p1.x;
  var dy = p2.y - p1.y;
