...
/* globals: _fm, filterInput, activeTags, (globalThis as any).currentChannel, parseFilterInput,
   _channelMatchesIsFlag */

export function matchesAllTags(tags, text) {
...
export function _isTagsFrom(allTags) {
  var out = [];
...
export function _matchAllWords(words, text) {
  return (
    words.length === 0 ||
...
export function filterSidebarElements(qWords, allTags) {
...
export function _currentInputTokens() {
...
export function _syncFilterChips() {
...
export function _toggleIsToken(flag) {
...
export function _initFilterChips() {
  var bar = document.getElementById("sidebar-filter-chips");
  if (!bar) return;
