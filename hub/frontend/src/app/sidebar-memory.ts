// @ts-nocheck
import { _topoPersistActiveMemSlot, _topoPoolMemoryDelete, _topoPoolMemoryRecall, _topoPoolMemoryRename, _topoPoolMemorySave, _topoPoolSelectAll, _topoPoolSelectClear } from "../activity-tab/multiselect";
import { _TOPO_POOL_MEM_MAX, _topoPoolMemories } from "../activity-tab/state";
import { _memSaveActionHandler, _memSelectOnChange, _sidebarMemoryRefreshBothSurfaces } from "./sidebar-memory-handlers";
...
export function _wireSidebarMemory() {
  ...
  host.addEventListener("click", function (ev) {
    ...
    if (chip && host.contains(chip)) {
      ...
      if (ev.shiftKey) {
        ...
        if ((globalThis as any)._topoActiveMemSlot === slotN) {
