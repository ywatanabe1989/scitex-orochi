...
import { cacheChannelIdentity, cachedAgentIcons, cachedChannelColors, cachedChannelIcons, channelIdentity } from "../agent-icons";
import { _addDragAndDrop } from "./agent-actions";
import { _addChannelContextMenu } from "./context-menus";
import { _channelDescriptions, _channelPrefs, _updateChannelTopicBanner } from "./members";
...
export function _setChannelPref(ch, patch) {
...
export function _setAgentIcon(name, patch) {
  if (
...
export function _setChannelIcon(ch, patch) {
  var normCh = ch.charAt(0) === "#" ? ch : "#" + ch;
  if (typeof cacheChannelIdentity === "function") {
...
export function _sortedStarred() {
...
export function _renderStarredSection() {
  ...
  container.innerHTML = starred
    .map(function (ch) {
      var active = (globalThis as any).currentChannel === ch ? " active" : "";
      var muted =
