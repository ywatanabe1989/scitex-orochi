// @ts-nocheck
import { _updateComposerTarget, lastActiveChannel } from "./state";
import { apiUrl, cleanAgentName, escapeHtml, orochiHeaders } from "./utils";
import { activeTab } from "../tabs";

/* ── Channel topic banner + subscriber list (todo#402) ── */
export var _channelDescriptions = {}; /* cache: channel → description */
export var _agentChannelMap = {}; /* cache: channel → [{name, online}] */
export var _channelPrefs =
  {}; /* cache: channel → {is_starred, is_muted, is_hidden, notification_level} */

export function _updateChannelTopicBanner(ch) {
...
export var _membersCache = {}; /* channel → [{username, permission, kind}] */

export function toggleMembersPanel() {
  var panel = document.getElementById("channel-members-panel");
  if (!panel) return;
  if (panel.style.display === "none") {
    openMembersPanel((globalThis as any).currentChannel || lastActiveChannel);
  } else {
    closeMembersPanel();
  }
}

export function openMembersPanel(ch) {
  var panel = document.getElementById("channel-members-panel");
...
export function _renderMembersPanel(members) {
  var list = document.getElementById("ch-members-list");
  if (!list) return;
...
export function _rebuildAgentChannelMap(agents) {
  var map = {};
  ...
  if ((globalThis as any).currentChannel) _updateChannelTopicBanner((globalThis as any).currentChannel);
}

export function openChannelTopicEdit() {
  var modal = document.getElementById("channel-topic-modal");
  var inp = document.getElementById("channel-topic-input");
  if (!modal || !inp) return;
  inp.value = _channelDescriptions[(globalThis as any).currentChannel] || "";
...
export function closeChannelTopicEdit() {
  var modal = document.getElementById("channel-topic-modal");
  if (modal) modal.style.display = "none";
}
window.closeChannelTopicEdit = closeChannelTopicEdit;

export function saveChannelTopic() {
  var inp = document.getElementById("channel-topic-input");
  if (!inp || !(globalThis as any).currentChannel) return;
  ...
  fetch(apiUrl("/api/channels/"), {
    ...
    body: JSON.stringify({ name: (globalThis as any).currentChannel, description: desc }),
    credentials: "same-origin",
  })
    .then(function (r) {
      return r.json();
    ...
    .then(function () {
      _channelDescriptions[(globalThis as any).currentChannel] = desc;
      _updateChannelTopicBanner((globalThis as any).currentChannel);
...
if ((globalThis as any).currentChannel) {
  try {
    ...
    if (_inp)
      _inp.placeholder =
        "Message " + (globalThis as any).currentChannel.replace(/^#/, "#") + "\u2026";
  } catch (_) {}
  document.addEventListener("DOMContentLoaded", function () {
    _updateComposerTarget((globalThis as any).currentChannel, false);
  });
}
