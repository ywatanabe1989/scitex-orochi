...
async function subscribeToPush() {
  ...
  if (!vapidKey) {
    console.error("No VAPID key available -- push not configured on server");
    return false;
  }
  ...
  try {
    ...
    var res = await fetch("/api/push/subscribe?token=" + token, {
      method: "POST",
    ...
    if (!res.ok) {
      console.error("Failed to send subscription to server:", res.status);
      return false;
    }
...
async function unsubscribeFromPush() {
  ...
  if (pushSubscription) {
    ...
    await fetch("/api/push/unsubscribe?token=" + token, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
