...
description: Fleet-wide quality monitoring procedures — smoke tests, CLI conventions, skill drift, regression watch. Owned by mamba-quality-checker.
---

# Quality Checks

Continuous quality monitoring across all SciTeX / Orochi packages. Owned by
**mamba-quality-checker** but any agent can run these checks on demand.

## Smoke Test (per-package)

The canonical smoke test lives at `scitex-agent-container/scripts/scitex-smoke-test.py`
(originally created by head-spartan in todo#194). It verifies, for every
...
mamba-quality-checker runs the smoke test:
...
`ywatanabe1989/todo` with the failing package, host, and traceback.

## CLI Convention Audit

Track which packages comply with the CLI conventions in `cli-conventions.md`:
- `--help` exits 0
...
Defer to `mamba-skill-manager` for the canonical drift audit, but
mamba-quality-checker should flag any obviously stale or contradictory skill
file it encounters during reviews and ping `@mamba-skill-manager`.

## Regression Watch (Pre-Release Gate)

Before any package is tagged + released (see `deploy-workflow.md` Release
Hygiene), mamba-quality-checker should:
...
mamba-quality-checker reports in:
