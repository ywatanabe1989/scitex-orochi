...
- [caduceus-healer-design](caduceus-healer-design.md) — Design doc for `todo#146` caduceus agent healer. 2-layer split: process-layer `caduceus-prober` daemon (quota-zero, 30s cadence, host-native scheduler, multi-signal NDJSON: systemd + docker stats + cpu.pressure + MCP dedup + pane state) + agent-layer `mamba-healer-<host>` workers (existing, extended to read daemon NDJSON + breadcrumbs as primary input). Recovery authority (SIGINT / `/compact` / `screen -X stuff` / systemd restart / MCP process dedup) stays worker-side, daemon never acts. Spartan-specific constraint matrix (no sudo / no systemd --user / no cron / no docker / Lmod `Python/3.11.3` mandatory / `$HOME`-backed breadcrumbs). Never auto-kill duplicate Claude sessions (head-spartan concurrent-instance anti-pattern, formalised as scitex-orochi#144). Schema v2 spec for per-container docker floats aligned with mamba-healer-nas live probe (msgs #11730 / #11746 / #11748).
